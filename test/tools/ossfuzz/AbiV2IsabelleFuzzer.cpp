...
DEFINE_PROTO_FUZZER(Contract const& _contract)
{
	...
	if (!typeString.empty() && converter.coderFunction())
	{
		...
		// Compile contract
		pair<solidity::bytes, Json::Value> cdata = EvmoneUtility::compileContract(contractSource, ":C");

		// Invoke test function. We always call the second function from the
		// list of alphabetically sorted interface functions.
		evmc::result result = EvmoneUtility::deployAndExecute(
			hostContext,
			cdata.first,
			(++cdata.second.begin())->asString() + encodedData.substr(2, encodedData.size())
		);
		...
		if (result.status_code == EVMC_SUCCESS)
			solAssert(
				EvmoneUtility::isOutputExpected(result.output_data, result.output_size, expectedOutput),
